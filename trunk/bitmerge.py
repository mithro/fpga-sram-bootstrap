...
assert (length+bsize) <= flash_size, "Combined files sizes of %d would exceed flash capacity of %d bytes" % ((length+bsize), flash_size)
print "Merged user data begins at FLASH address 0x%06X" %length

#write recalculated section length
data = struct.pack(">L", length+bsize)
args.ofile.write(data)
